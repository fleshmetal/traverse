...
def main() -> None:
    ...
    ap.add_argument(
        "--cache-dir",
        required=True,
        help="Path containing canonical_plays.parquet and canonical_tracks.parquet",
    )
    ap.add_argument(
        "--sample", type=int, default=100000, help="Sample N rows for quick co-occurrence trial"
    )
    args = ap.parse_args()
    ...
    _status(
        f"plays[genres] non-null={plays_g_nonnull:,} | plays[styles] non-null={plays_s_nonnull:,}"
    )
    _status(
        f"tracks[genres] non-null={tracks_g_nonnull:,} | tracks[styles] non-null={tracks_s_nonnull:,}"
    )

    # If plays have no tags, try joining from tracks by track_id
    joined = plays.copy()
    if (
        (plays_g_nonnull == 0 and plays_s_nonnull == 0)
        and "track_id" in plays.columns
        and "track_id" in tracks.columns
    ):
        ...
        _status(
            f"joined[genres] non-null={j_g_nonnull:,} | joined[styles] non-null={j_s_nonnull:,}"
        )
    else:
    ...
    _status(
        f"DIAG: rows_with_any_tags(sample)={rows_with_any:,} | pairs_found(sample)={len(counts):,}"
    )
    if counts:
        ...
        _status(
            "No pairs in sample — root cause is likely empty tags on both plays & tracks or unexpected tag format."
        )
