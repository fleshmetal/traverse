...
def _normalize_row(r: dict) -> dict | None:
    ...
    artist_name = (
        r.get("master_metadata_album_artist_name") or r.get("artist_name") or r.get("artistName")
    )
...
def load_plays_from_extended(extended_dir: Path) -> pd.DataFrame:
    ...
    rows: List[dict] = []
    for row in tqdm(
        _iter_extended_json(files), total=len(files), desc="Reading Extended JSON", unit="file"
    ):
    ...
    if not rows:
        return pd.DataFrame(
            columns=["played_at", "track_id", "ms_played", "track_name", "artist_name"]
        )
...
def build_minimal_graph(
    plays: pd.DataFrame, agg: str = "play_count", min_weight: int = 1
    ...
    if agg == "ms_played":
        edges = (
            plays.groupby(["track_id", "artist_id"], dropna=False)["ms_played"]
            .sum()
            .reset_index(name="value")
        )
    else:
        edges = (
            plays.groupby(["track_id", "artist_id"], dropna=False).size().reset_index(name="value")
        )
    edges = edges[edges["value"] >= min_weight].copy()

    # nodes
    track_nodes = (
        plays.groupby(["track_id", "track_name"], dropna=False)
        .size()
        .reset_index()[["track_id", "track_name"]]
        .rename(columns={"track_id": "id", "track_name": "label"})
    )
    track_nodes["type"] = "track"

    artist_nodes = (
        plays.groupby(["artist_id", "artist_name"], dropna=False)
        .size()
        .reset_index()[["artist_id", "artist_name"]]
    ...
    edges = edges.rename(columns={"track_id": "source", "artist_id": "target"})[
        ["source", "target", "value"]
    ]
...
def write_cosmograph_csv(
    nodes: pd.DataFrame, edges: pd.DataFrame, out_dir: Path
) -> tuple[Path, Path]:
...
def main():
    ap = argparse.ArgumentParser(
        description="Export CSVs for Cosmograph web app (no bundler required)."
    )
    ...
    print(
        f"Loaded plays: rows={len(plays):,} [played_at, track_id, ms_played, track_name, artist_name]"
    )
    ...
    print("\nNext steps:")
    print(
        f"  1) Start a CORS static server in the output folder:\n"
        f"       cd {out_dir}\n"
